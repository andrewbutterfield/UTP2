...
--
-- Most importantly the given function to run, runs in the UTP2 monad.
...
-- | Similar to 'onChanges' but also runs once when initially called.
nowAndOnChange :: Behavior a -> (a -> UI void) -> UI ()
nowAndOnChange behavior f = do
  currentValue behavior >>= f
  onChanges behavior f
