...
dnfLists (Not pr)  = map (map pnot) (cnfLists pr)
dnfLists (And prs) = map concat (distribute (map dnfLists prs))
dnfLists (Or  prs) = concat  (map dnfLists prs)
dnfLists pr        = [[pr]]

cnfLists (Not pr)  = map (map pnot) (dnfLists pr)
cnfLists (And prs) = concat (map cnfLists prs)
cnfLists (Or  prs) = map concat (distribute (map cnfLists prs))
cnfLists pr        = [[pr]]
\end{code}
...
pushPF f (And prs) = And (map (pushPF f) prs)
pushPF f (Or prs)  = Or (map (pushPF f) prs)
pushPF f pr        = f pr
