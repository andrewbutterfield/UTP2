...
\begin{code}
tmmerge :: Monad m => (t -> t -> m t) -> Trie t -> Trie t -> m (Trie t)
...
\begin{code}
btmmerge :: Monad m
         => (t -> t -> m t) -> Btree Char (Trie t) -> Btree Char (Trie t)
         -> m (Btree Char (Trie t))

btmmerge mmrg Bnil btree2 = return btree2
btmmerge mmrg btree1 Bnil = return btree1
...
btmmerge mmrg bt1@(Bleaf k1 trie1) (Bleaf k2 trie2)
 ...
 | k1 <  k2   =  return (Bbr k2 trie2 bt1 Bnil)

 | otherwise  =  return (Bbr k2 trie2 Bnil bt1)
...
\begin{code}
tglue :: (Monad m, Eq t) => Trie t -> Trie t -> m (Trie t)
tglue t1 t2 = tmmerge trieglue t1 t2

trieglue :: (Monad m, Eq t) => t -> t -> m t
trieglue a b
    | (a == b)   =  return a
    | otherwise  =  fail "trieglue: conflicting entries"

tglueW :: Monad m => (t -> t -> Bool) -> Trie t -> Trie t -> m (Trie t)
tglueW fits t1 t2 = tmmerge (tglueWith fits) t1 t2

tglueWith :: Monad m => (t -> t -> Bool) -> t -> t -> m t
tglueWith fits a b
    | (a `fits` b)  =  return a
    | otherwise     =  fail "tglueWith: conflicting entries"
\end{code}
