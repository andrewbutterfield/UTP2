...
two levels of failure, denoted by \texttt{Nothing} and \texttt{Just (False,_)}.
\begin{code}
...
putting a list, one element per line, with indenting
\begin{code}
...
\begin{code}
pseqsub :: (a -> b -> Bool) -> [b] -> [a] -> [b]
pseqsub eq xs [] = xs
pseqsub eq (x:xs) (y:ys)
 | y `eq` x  =  pseqsub eq xs ys
...
isPrefixOfBy eq [] _         =  True
isPrefixOfBy eq _  []        =  False
isPrefixOfBy eq (x:xs) (y:ys)=  x `eq` y && isPrefixOfBy eq xs ys
\end{code}

\begin{code}
splitPrefixBy :: (a -> b -> Bool) -> [a] -> [b] -> Maybe ([b],[b])
splitPrefixBy eq = split []
 where
   split pre [] post  =  Just (reverse pre, post)
   split _   _  []    =  Nothing
   split pre (x:xs) (y:ys)
    | x `eq` y        =  split (y:pre) xs ys
    | otherwise       =  Nothing
\end{code}

Testing for a (the first) contiguous subsequence, returning it and prior and
subsequent portions if found:
\begin{code}
locateSubSeqBy :: (a -> b -> Bool) -> [a] -> [b] -> Maybe ([b],[b],[b])
locateSubSeqBy eq [] seq          =  Nothing
locateSubSeqBy eq sub seq         =  locate sub [] seq
 where
   locate sub before []           =  Nothing
   locate sub before after@(x:xs)
    = case splitPrefixBy eq sub after of
       Nothing                   ->  locate sub (x:before) xs
       Just (pre,post)           ->  Just (reverse before, pre, post)

locateSubSeq :: Eq a => [a] -> [a] -> Maybe ([a],[a],[a])
locateSubSeq = locateSubSeqBy (==)
\end{code}
...
either my assuming positive numbers only,
or a bounded type
\begin{code}
